...
|  --- | --- | --- | --- | --- | --- |
| `actionList` | `?(string[])` | Optional | Array of actions (one or more) to be included in the payment to invoke bundled services along with payment status.<br><br>Possible values are one or more of follows:<br><br>- `AP_REFUND`: Use this when Alternative Payment Refund service is requested. | getActionList(): ?array | setActionList(?array actionList): void |
...
| `paymentType` | `?string` | Optional | Identifier for the payment type | getPaymentType(): ?string | setPaymentType(?string paymentType): void |
...
$processingInformation7 = ProcessingInformation7Builder::init()
    ->actionList(
        [
            'actionList9',
            'actionList0',
    ...
    ->paymentSolution('paymentSolution0')
    ->reconciliationId('reconciliationId0')
    ->linkId('linkId4')
    ->reportGroup('reportGroup8')
