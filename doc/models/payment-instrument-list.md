...
| `count` | `?int` | Optional, Read-only | The number of Payment Instruments returned in the array. | getCount(): ?int | setCount(?int count): void |
| `total` | `?int` | Optional, Read-only | The total number of Payment Instruments associated with the Customer or Instrument Identifier. | getTotal(): ?int | setTotal(?int total): void |
| `embedded` | [`?Embedded12`](../../doc/models/embedded-12.md) | Optional, Read-only | - | getEmbedded(): ?Embedded12 | setEmbedded(?Embedded12 embedded): void |
| `additionalProperties` | `array<string, array>` | Optional | - | findAdditionalProperty(string key): array | additionalProperty(string key, array value): void |

## Example

```php
use VisaAcceptanceMergedSpecLib\Models\Builders\PaymentInstrumentListBuilder;
...
$paymentInstrumentList = PaymentInstrumentListBuilder::init()
    ->links(
        Links66Builder::init()
            ->self(
                Self65Builder::init()
